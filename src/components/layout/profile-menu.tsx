...
export function ProfileMenu({
  ...
  return (
    <div className="relative" ref={rootRef}>
      ...
      {open ? (
        <div className="fixed inset-0 z-[60] max-[860px]:flex max-[860px]:items-end min-[861px]:hidden" role="dialog" aria-modal="true" aria-label="Profile">
          <button
            type="button"
            aria-label="Close profile"
            className="absolute inset-0 cursor-default bg-[rgb(15_15_18/40%)]"
          ...
          <div className="relative w-full rounded-t-[1.6rem] border-t border-border bg-surface p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-[0_-10px_40px_rgb(15_15_18/20%)] profile-curtain-in">
            ...
            <a
              className="flex min-h-[3rem] w-full items-center gap-[.6rem] rounded-[.7rem] border-0 bg-transparent px-[.7rem] py-[.55rem] text-left text-[.9rem] font-medium text-foreground hover:bg-surface-subtle focus-visible:bg-surface-subtle"
