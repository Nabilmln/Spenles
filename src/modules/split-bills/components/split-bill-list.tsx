...
export function SplitBillList({
  ...
  if (!rows.length) {
    ...
    return (
      <EmptyState
        ...
        title={
          hasFilters ? "No bills for these filters" : "No Split Bill history yet"
        }
        description={
          hasFilters
            ? "Try changing the keyword or filters used."
            : "Create your first Split Bill and the split history will be saved here."
        }
        action={
          hasFilters ? (
  ...
  return (
    <>
          ...
          <article className={`${cardClass} grid min-w-0 gap-[.75rem]`} key={row.id}>
            ...
            <div className="flex items-baseline justify-between gap-3">
              ...
              <strong className="min-w-0 text-[.95rem] [overflow-wrap:anywhere]">
                {row.finalAmount ? formatIdr(row.finalAmount) : "Not finalized"}
              </strong>
            </div>
      ...
      <nav aria-label="Split bill pagination" className="flex flex-col items-center gap-2">
        ...
        <p className="m-0 text-[.76rem] text-muted">
          Page {filters.page} of {totalPages} · {total} bills
        </p>
      </nav>
    </>
  );
